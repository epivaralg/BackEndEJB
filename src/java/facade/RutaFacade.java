...
public class RutaFacade extends AbstractFacade<Ruta> {
    ...
    public boolean isIdMunicipioEmpty(Ruta entity) {
        ...
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(ruta, entity), cb.isNotNull(ruta.get(Ruta_f.idMunicipio)));
    ...
    public boolean isIdSolicitudEmpty(Ruta entity) {
        ...
        CriteriaQuery<Long> cq = cb.createQuery(Long.class);
        Root<Ruta> ruta = cq.from(Ruta.class);
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(ruta, entity), cb.isNotNull(ruta.get(Ruta_f.idSolicitud)));
        return em.createQuery(cq).getResultList().isEmpty();
    }
