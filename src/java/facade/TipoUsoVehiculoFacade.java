...
public class TipoUsoVehiculoFacade extends AbstractFacade<TipoUsoVehiculo> {
    ...
    public boolean isSolicitudCollectionEmpty(TipoUsoVehiculo entity) {
        ...
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(tipoUsoVehiculo, entity), cb.isNotEmpty(tipoUsoVehiculo.get(TipoUsoVehiculo_f.solicitudCollection)));
