...
public class RazonEstadoFacade extends AbstractFacade<RazonEstado> {
    ...
    public boolean isHistoriaEstadoCollectionEmpty(RazonEstado entity) {
        ...
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(razonEstado, entity), cb.isNotEmpty(razonEstado.get(RazonEstado_f.historiaEstadoCollection)));
    ...
    public boolean isIdEstadoSolicitudEmpty(RazonEstado entity) {
        ...
        CriteriaQuery<Long> cq = cb.createQuery(Long.class);
        Root<RazonEstado> razonEstado = cq.from(RazonEstado.class);
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(razonEstado, entity), cb.isNotNull(razonEstado.get(RazonEstado_f.idEstadoSolicitud)));
        return em.createQuery(cq).getResultList().isEmpty();
    }
