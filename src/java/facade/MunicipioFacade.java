...
public class MunicipioFacade extends AbstractFacade<Municipio> {
    ...
    public boolean isRutaCollectionEmpty(Municipio entity) {
        ...
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(municipio, entity), cb.isNotEmpty(municipio.get(Municipio_f.rutaCollection)));
    ...
    public boolean isIdDepartamentoEmpty(Municipio entity) {
        ...
        CriteriaQuery<Long> cq = cb.createQuery(Long.class);
        Root<Municipio> municipio = cq.from(Municipio.class);
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(municipio, entity), cb.isNotNull(municipio.get(Municipio_f.idDepartamento)));
        return em.createQuery(cq).getResultList().isEmpty();
    }
