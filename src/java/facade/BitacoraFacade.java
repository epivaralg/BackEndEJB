...
public class BitacoraFacade extends AbstractFacade<Bitacora> {
    ...
    public boolean isIdTipoBitacoraEmpty(Bitacora entity) {
        ...
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(bitacora, entity), cb.isNotNull(bitacora.get(Bitacora_f.idTipoBitacora)));
    ...
    public boolean isIdUsuarioEmpty(Bitacora entity) {
        ...
        CriteriaQuery<Long> cq = cb.createQuery(Long.class);
        Root<Bitacora> bitacora = cq.from(Bitacora.class);
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(bitacora, entity), cb.isNotNull(bitacora.get(Bitacora_f.idUsuario)));
        return em.createQuery(cq).getResultList().isEmpty();
    }
