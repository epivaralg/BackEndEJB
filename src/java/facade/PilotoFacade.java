...
public class PilotoFacade extends AbstractFacade<Piloto> {
    ...
    public boolean isComisionCollectionEmpty(Piloto entity) {
        ...
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(piloto, entity), cb.isNotEmpty(piloto.get(Piloto_f.comisionCollection)));
    ...
    public boolean isIdEstadoPilotoEmpty(Piloto entity) {
        ...
        CriteriaQuery<Long> cq = cb.createQuery(Long.class);
        Root<Piloto> piloto = cq.from(Piloto.class);
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(piloto, entity), cb.isNotNull(piloto.get(Piloto_f.idEstadoPiloto)));
        return em.createQuery(cq).getResultList().isEmpty();
    }
    ...
    public boolean isIdGrupoEmpty(Piloto entity) {
        ...
        CriteriaQuery<Long> cq = cb.createQuery(Long.class);
        Root<Piloto> piloto = cq.from(Piloto.class);
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(piloto, entity), cb.isNotNull(piloto.get(Piloto_f.idGrupo)));
        return em.createQuery(cq).getResultList().isEmpty();
    }
    ...
    public boolean isLicenciaCollectionEmpty(Piloto entity) {
        ...
        CriteriaQuery<Long> cq = cb.createQuery(Long.class);
        Root<Piloto> piloto = cq.from(Piloto.class);
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(piloto, entity), cb.isNotEmpty(piloto.get(Piloto_f.licenciaCollection)));
        return em.createQuery(cq).getResultList().isEmpty();
    }
