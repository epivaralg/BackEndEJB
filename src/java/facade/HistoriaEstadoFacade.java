...
public class HistoriaEstadoFacade extends AbstractFacade<HistoriaEstado> {
    ...
    public boolean isIdRazonEstadoEmpty(HistoriaEstado entity) {
        ...
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(historiaEstado, entity), cb.isNotNull(historiaEstado.get(HistoriaEstado_f.idRazonEstado)));
    ...
    public boolean isIdSolicitudEmpty(HistoriaEstado entity) {
        ...
        CriteriaQuery<Long> cq = cb.createQuery(Long.class);
        Root<HistoriaEstado> historiaEstado = cq.from(HistoriaEstado.class);
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(historiaEstado, entity), cb.isNotNull(historiaEstado.get(HistoriaEstado_f.idSolicitud)));
        return em.createQuery(cq).getResultList().isEmpty();
    }
