...
public class UnidadFacade extends AbstractFacade<Unidad> {
    ...
    public boolean isIdOficinaEmpty(Unidad entity) {
        ...
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(unidad, entity), cb.isNotNull(unidad.get(Unidad_f.idOficina)));
    ...
    public boolean isUsuarioCollectionEmpty(Unidad entity) {
        ...
        CriteriaQuery<Long> cq = cb.createQuery(Long.class);
        Root<Unidad> unidad = cq.from(Unidad.class);
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(unidad, entity), cb.isNotEmpty(unidad.get(Unidad_f.usuarioCollection)));
        return em.createQuery(cq).getResultList().isEmpty();
    }
