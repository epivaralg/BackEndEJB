...
public class VehiculoFacade extends AbstractFacade<Vehiculo> {
    ...
    public boolean isComisionCollectionEmpty(Vehiculo entity) {
        ...
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(vehiculo, entity), cb.isNotEmpty(vehiculo.get(Vehiculo_f.comisionCollection)));
    ...
    public boolean isIdEstadoVehiculoEmpty(Vehiculo entity) {
        ...
        CriteriaQuery<Long> cq = cb.createQuery(Long.class);
        Root<Vehiculo> vehiculo = cq.from(Vehiculo.class);
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(vehiculo, entity), cb.isNotNull(vehiculo.get(Vehiculo_f.idEstadoVehiculo)));
        return em.createQuery(cq).getResultList().isEmpty();
    }
    ...
    public boolean isIdLineaEmpty(Vehiculo entity) {
        ...
        CriteriaQuery<Long> cq = cb.createQuery(Long.class);
        Root<Vehiculo> vehiculo = cq.from(Vehiculo.class);
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(vehiculo, entity), cb.isNotNull(vehiculo.get(Vehiculo_f.idLinea)));
        return em.createQuery(cq).getResultList().isEmpty();
    }
    ...
    public boolean isIdTipoVehiculoEmpty(Vehiculo entity) {
        ...
        CriteriaQuery<Long> cq = cb.createQuery(Long.class);
        Root<Vehiculo> vehiculo = cq.from(Vehiculo.class);
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(vehiculo, entity), cb.isNotNull(vehiculo.get(Vehiculo_f.idTipoVehiculo)));
        return em.createQuery(cq).getResultList().isEmpty();
    }
