...
public class TipoLicenciaFacade extends AbstractFacade<TipoLicencia> {
    ...
    public boolean isLicenciaCollectionEmpty(TipoLicencia entity) {
        ...
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(tipoLicencia, entity), cb.isNotEmpty(tipoLicencia.get(TipoLicencia_f.licenciaCollection)));
