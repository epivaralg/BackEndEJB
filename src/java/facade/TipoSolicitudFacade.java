...
public class TipoSolicitudFacade extends AbstractFacade<TipoSolicitud> {
    ...
    public boolean isSolicitudCollectionEmpty(TipoSolicitud entity) {
        ...
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(tipoSolicitud, entity), cb.isNotEmpty(tipoSolicitud.get(TipoSolicitud_f.solicitudCollection)));
