...
public class OficinaFacade extends AbstractFacade<Oficina> {
    ...
    public boolean isIdDepartamentoEmpty(Oficina entity) {
        ...
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(oficina, entity), cb.isNotNull(oficina.get(Oficina_f.idDepartamento)));
    ...
    public boolean isUnidadCollectionEmpty(Oficina entity) {
        ...
        CriteriaQuery<Long> cq = cb.createQuery(Long.class);
        Root<Oficina> oficina = cq.from(Oficina.class);
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(oficina, entity), cb.isNotEmpty(oficina.get(Oficina_f.unidadCollection)));
        return em.createQuery(cq).getResultList().isEmpty();
    }
