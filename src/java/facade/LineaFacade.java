...
public class LineaFacade extends AbstractFacade<Linea> {
    ...
    public boolean isIdMarcaEmpty(Linea entity) {
        ...
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(linea, entity), cb.isNotNull(linea.get(Linea_f.idMarca)));
    ...
    public boolean isVehiculoCollectionEmpty(Linea entity) {
        ...
        CriteriaQuery<Long> cq = cb.createQuery(Long.class);
        Root<Linea> linea = cq.from(Linea.class);
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(linea, entity), cb.isNotEmpty(linea.get(Linea_f.vehiculoCollection)));
        return em.createQuery(cq).getResultList().isEmpty();
    }
