...
public class LicenciaFacade extends AbstractFacade<Licencia> {
    ...
    public boolean isIdPilotoEmpty(Licencia entity) {
        ...
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(licencia, entity), cb.isNotNull(licencia.get(Licencia_f.idPiloto)));
    ...
    public boolean isIdTipoLicenciaEmpty(Licencia entity) {
        ...
        CriteriaQuery<Long> cq = cb.createQuery(Long.class);
        Root<Licencia> licencia = cq.from(Licencia.class);
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(licencia, entity), cb.isNotNull(licencia.get(Licencia_f.idTipoLicencia)));
        return em.createQuery(cq).getResultList().isEmpty();
    }
