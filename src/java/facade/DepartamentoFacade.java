...
public class DepartamentoFacade extends AbstractFacade<Departamento> {
    ...
    public boolean isOficinaCollectionEmpty(Departamento entity) {
        ...
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(departamento, entity), cb.isNotEmpty(departamento.get(Departamento_f.oficinaCollection)));
    ...
    public boolean isMunicipioCollectionEmpty(Departamento entity) {
        ...
        CriteriaQuery<Long> cq = cb.createQuery(Long.class);
        Root<Departamento> departamento = cq.from(Departamento.class);
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(departamento, entity), cb.isNotEmpty(departamento.get(Departamento_f.municipioCollection)));
        return em.createQuery(cq).getResultList().isEmpty();
    }
