...
public class EstadoVehiculoFacade extends AbstractFacade<EstadoVehiculo> {
    ...
    public boolean isVehiculoCollectionEmpty(EstadoVehiculo entity) {
        ...
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(estadoVehiculo, entity), cb.isNotEmpty(estadoVehiculo.get(EstadoVehiculo_f.vehiculoCollection)));
