...
public class EstadoSolicitudFacade extends AbstractFacade<EstadoSolicitud> {
    ...
    public boolean isRazonEstadoCollectionEmpty(EstadoSolicitud entity) {
        ...
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(estadoSolicitud, entity), cb.isNotEmpty(estadoSolicitud.get(EstadoSolicitud_f.razonEstadoCollection)));
